...
export async function GET(context: APIContext) {
  ...
  const markup = html`
    <div
      tw="bg-zinc-900 flex flex-col w-full h-full rounded-lg overflow-hidden shadow-lg text-white border border-zinc-700/50"
    >
      ...
      >
        ...
        <div tw="flex items-center bg-zinc-800/50 rounded-lg px-4 py-2">
          <img src="https://i.imgur.com/Njcm9JS.png" tw="w-15 h-15" />
          <div tw="flex flex-col ml-4 border-l border-zinc-700/50 pl-4">
            <span tw="text-zinc-400 font-semibold">Harta Angkasa</span>
            <span tw="text-zinc-400 text-sm">ItsHarta</span>
          </div>
        </div>
      </div>
    </div>
